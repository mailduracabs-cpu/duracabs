            ...
            >
                @forelse ($products as $vehicle)
                    ...
                    >
                        <button
                            type="button"
                            x-on:click="window.dispatchEvent(new CustomEvent('open-self-drive-popup', { detail: { vehicleId: {{ $vehicle->id }}, vehicleName: @js($vehicleName), vehicleImage: @js($vehicleImage), hourlyPrice: {{ $hourlyPrice }}, minimumHours: {{ $minimumBookingHours }}, securityDeposit: {{ $securityDeposit }} } }))"
                            class="group relative block aspect-[16/10] w-full overflow-hidden bg-slate-100 text-left"
                            aria-label="Choose date and time for {{ $vehicleName }}"
                        >
                            <img
                                src="{{ $vehicleImage }}"
