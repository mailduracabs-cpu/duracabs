<div class="w-full max-w-[70rem] lg:px-10 px-4 mt-2  mx-auto">
 ...
 @section('title', $ride->meta_title)

  @section('description',$ride->meta_description)
 ...
 @if ($tab === 'one_way')
    ...
    @if ($tab === 'self_drive')
        <div class="fixed z-50 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full -top-20 ">
            <div class="relative top-40 mx-auto shadow-xl rounded-md bg-white max-w-md">
                ...
                <form
                    wire:submit.prevent='submitSelfDrive([{{ $ride->id }},"{{ $time }}","{{ $endTime }}","{{ $tab }}","{{ $date }}","{{ $endDate }}",{{ $ride->price * $hours }}, "{{ $security }}"])'>
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="flex">
                ...
                <div class="w-full px-4 md:w-1/2 ">
                    <div class="lg:pl-20">
                        ...
                        @if ($ride->ride_type === 'self_drive')
                            <div class="flex flex-wrap items-center gap-4">
                                <button wire:click='tabValue(["self_drive","{{ $ride->price }}","{{ $ride->name }}","{{ $ride->category->name }}"])'
                                    class="w-full p-4 bg-sky-500 rounded-md lg:w-2/5 dark:text-gray-200 text-gray-50 hover:bg-blue-600 dark:bg-blue-500 dark:hover:bg-blue-700">
                        ...
                        @foreach ($prices as $price)
                            <div
                                ...
                                <div class="lg:flex  justify-between">
                                    <div class="relative ">
                                        <a href="/route/{{ $ride->slug }}" class="flex items-center justify-center h-full w-full">
                                        ...
                                        <div
                                            ...
                                            <div class="px-2 grid grid-rows-1 gap-0 place-items-center mt-0 ">
                                                ...
                                                <p class="text-[10px] leading-4 text-center w-3/4 mt-2">
                                                   {{ $price->category->passanger_capacity }} {{ $price->category->model }} 
                                                </p>
                                            </div>
                                            <div class="">
                                    ...
                                    <div class="p-3 text-center">
                                        ...
                                        <div class="text-lg text-center">
                                            ...
                                            <p class="text-black-600 text-xm font-bold dark:text-black-600">
                                                {{ Number::currency($price->price, 'INR') }}{{$ride->ride_type === 'self_drive'? 'Per Hour' : ''}}</p>
                                        </div>
                                        ...
                                        @if ($ride->ride_type === 'one_way')
                                            ...
                                            <div class="mt-2">
                                                <button onclick="showFareSummaryOneWay('{{ $ride->name }}', '{{ $price->category->name }}', {{ $price->price }}, {{ $price->max_price }}, {{ $ride->toll_tax }}, {{ $ride->km_limit }}, {{ $ride->hr_limit }}, {{ $ride->extra_km_charge }}, {{ $ride->extra_hr_charge }})" 
                                        ...
                                        @if ($ride->ride_type === 'local')
                                            ...
                                            <div class="mt-2">
                                                <button onclick="showFareSummaryLocal('{{ $ride->name }}', '{{ $price->category->name }}', {{ $price->price }}, {{ $price->max_price }}, 1, 'Local Package', {{ $ride->extra_km_charge }}, {{ $ride->extra_hr_charge }}, {{ $ride->driver_allowances }})" 
                                        ...
                                        @endif
                                         ...
                                         @if ($ride->ride_type === 'self_drive')
                                            <div class="flex flex-wrap items-center gap-4">
                                                <button
                                                    wire:click='tabValue(["self_drive","{{ $price->price }}","{{ $ride->name }}","{{ $price->category->name }}","{{ $price->category->security ?? 0 }}"])'
                                                    class="w-full p-1 bg-sky-500 rounded-full  dark:text-gray-200 text-gray-50 hover:bg-blue-600 dark:bg-blue-500 dark:hover:bg-blue-700">
                                                    <span wire:loading.remove wire:target='tabValue("one_way")'>
                                                        Book Now
                                                    </span>
                                            ...
                                            <div class="mt-2">
                                                <button onclick="showFareSummarySelfDrive('{{ $ride->name }}', '{{ $price->category->name }}', {{ $price->price }}, {{ $price->max_price }}, 1, '{{ $price->category->security ?? 0 }}')" 
    ...
    <section class=" font-poppins dark:bg-gray-800 rounded-lg">
        ...
        <div class="bg-white p-0 mt-0 ">
            ...
            <h1 class="text-3xl mt-5 font-bold">{{$ride->name }} One Way Cab & Taxi Service – Book Affordable Ride with DuraCabs</h1>
            ...
            <div class="description mt-2">
               ...
               <h2> Quick Facts about {{$ride->name}}</h2>

                <p><strong>Destination</strong> :{{$ride->name}} </p>

               <p><strong>{{$ride->name}} Distance Oneway/One Side:</strong></p>

                <p> <strong>Route 1</strong> : {{$ride->hr_limit}} Hour  ({{$ride->km_limit}} KM) via NE 4</p>
                    ...
                    <tbody>
                        ...
                        @foreach ($prices as $item)
                            <tr class="border-b border-gray-200 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4">
                                    {{ $item->category->luggage_capacity }} Bags
                                </td>
                                <td class="px-6 py-4">
                                    {{ Number::currency($item->category->km_charge, 'INR') }}
                                </td>
                                <td class="px-6 py-4">
        ...
        <div class="p-2 bg-white mt-3">
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-2">
                <h3 class=" font-normal p-2 text-xm">Here are the reasons why you should choose Duracabs in {{$ride->name }}?. </h3>
