                ...
                <main class="product-content w-full px-3 lg:w-3/4">
                    <div class="mb-4">
                        <div
                            ...
                            <div><p class="text-sm font-extrabold text-slate-900">{{ count($prices) }} Cab Packages Available</p><p class="text-xs text-slate-500">Ride page jaisa premium vehicle selection</p></div>
                            {{-- <div class="flex items-center justify-between">
                            ...
                            <article wire:key="product-price-{{ $price->id ?? $loop->index }}" class="ride-package-card product-ride-card">
                                ...
                                <div class="ride-package-price">
                                    ...
                                    @if ($fareUnlocked)
                                        ...
                                        @else
                                            <button type="button"
                                                onclick="showFareSummarySelfDrive('{{ addslashes($ride->name) }}', '{{ addslashes($price->category->name) }}', {{ $price->price }}, {{ $price->max_price }}, 1, '{{ $price->category->security ?? 0 }}')"
                                                class="ride-fare-icon-button">
                                                <i class="fa-solid fa-circle-info"></i><span>Fare details</span>
                                            </button>
                                        @endif
