<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
    ...
    @section('description', 'Manage your Duracabs account, view orders, and update profile information')
    ...
    <div class="border-b border-gray-200 dark:border-gray-700 mb-8">
        <nav class="flex space-x-8" aria-label="Tabs">
            ...
            <button wire:click="changeTab('profile')" 
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'profile' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                <svg class="w-5 h-5 inline-block mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button wire:click="changeTab('address')" 
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'address' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                <svg class="w-5 h-5 inline-block mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button wire:click="changeTab('orders')" 
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'orders' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                <svg class="w-5 h-5 inline-block mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button wire:click="changeTab('security')" 
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'security' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                <svg class="w-5 h-5 inline-block mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-4 md:p-5 flex gap-x-4">
                    ...
                    <div class="grow">
                        ...
                        <div class="mt-1 flex items-center gap-x-2">
                            <h3 class="text-xl font-medium text-gray-800 dark:text-gray-200">{{ $user->name }}</h3>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-slate-900 dark:border-gray-800">
                <div class="p-4 md:p-5 flex gap-x-4">
                    ...
                    <div class="grow">
                        ...
                        <div class="mt-1 flex items-center gap-x-2">
                            <h3 class="text-xl font-medium text-gray-800 dark:text-gray-200">{{ $totalOrders }}</h3>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-slate-900 dark:border-gray-800">
                <div class="p-4 md:p-5 flex gap-x-4">
                    ...
                    <div class="grow">
                        ...
                        <div class="mt-1 flex items-center gap-x-2">
                            <h3 class="text-sm font-medium text-gray-800 dark:text-gray-200">{{ $user->mobile }}</h3>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg dark:bg-gray-800">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">
                                    {{ $order->booking_from }} → {{ $order->booking_to }}
                                </p>
                        ...
                        <div class="flex items-center space-x-2">
                            <span class="px-2 py-1 text-xs font-semibold rounded-full 
                                {{ $order->status === 'confirm' ? 'bg-green-100 text-green-800' : 
                                   ($order->status === 'cancelled' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800') }}">
                                {{ ucfirst($order->status) }}
                            </span>
                            <span class="text-sm font-medium text-gray-900 dark:text-white">₹{{ number_format($order->grand_total) }}</span>
                        </div>
                    </div>
                    @endforeach
                </div>
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-gray-900 dark:text-white">{{ $user->name ?: 'Not provided' }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="text-gray-900 dark:text-white">{{ $user->mobile ?: 'Not provided' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Company Name</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->company_name ?: 'Not provided' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">GST Number</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->gst_number ?: 'Not provided' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Aadhar Number</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->aadhar_number ? '****-****-' . substr($user->aadhar_number, -4) : 'Not provided' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Driving License</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->driving_licence_number ?: 'Not provided' }}</p>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Office Address</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->office_address ?: 'Not provided' }}</p>
                </div>
            </div>
            @else
            <!-- Edit Mode -->
            <form wire:submit="updateProfile" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @error('aadhar_number') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        ...
                        @error('driving_licence_number') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    <div class="md:col-span-2">
                    ...
                    <div>
                        ...
                        @error('gst_image') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        ...
                        @error('aadhar_image') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                </div>
    ...
    <div class="bg-white border shadow-sm rounded-xl dark:bg-slate-900 dark:border-gray-800">
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Full Name</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->address->full_name }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->address->email }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Phone</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->address->phone }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Alternate Phone</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->address->phone2 ?: 'Not provided' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">City</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->address->city }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">State</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->address->state }}</p>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Pickup Address</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->address->pickup_address }}</p>
                </div>
                @if($user->address->drop_address)
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Drop Address</label>
                    <p class="text-gray-900 dark:text-white">{{ $user->address->drop_address }}</p>
                </div>
                @endif
            </div>
            @elseif(!$editingAddress && !$user->address)
            <!-- No Address State -->
            ...
            <form wire:submit="updateAddress" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @error('address_email') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        @error('pickup_address') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    <div class="md:col-span-2">
                        ...
                        @error('drop_address') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition duration-200">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">Order #{{ $order->id }} • {{ $order->created_at->format('M d, Y h:i A') }}</p>
                                <p class="text-sm text-gray-600">{{ ucfirst($order->ride_type) }} • {{ $order->taxi_type }}</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <p class="text-lg font-semibold text-gray-900 dark:text-white">₹{{ number_format($order->grand_total) }}</p>
                    ...
                    <div class="mt-4 flex justify-between items-center">
                        <div class="text-sm text-gray-500">
                            <span>Payment: {{ ucfirst($order->payment_method) }}</span>
                        </div>
                        <a href="/my-orders/{{ $order->id }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                            View Details →
                        </a>
                    </div>
                </div>
