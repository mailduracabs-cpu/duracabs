...
class ServiceSearchPanel extends Component
{
    ...
    private const ALLOWED_TABS = ['one_way', 'return', 'local', 'self_drive'];
    ...
    public function mount(
        ...
        $this->defaultTab = in_array($defaultTab, self::ALLOWED_TABS, true)
            ? $defaultTab
            : self::DEFAULT_TAB;
        $this->defaultFromCity = filled($defaultFromCity) ? trim($defaultFromCity) : null;
        $this->defaultToCity = filled($defaultToCity) ? trim($defaultToCity) : null;
        ...
        if ($this->defaultFromCity !== null) {
            ...
            $this->queryFrom = $this->defaultFromCity;
            $this->queryFrom_search = $this->defaultFromCity;
            $this->queryLocal = $this->defaultFromCity;
            $this->querySelfDrive = $this->defaultFromCity;
        }

        if ($this->defaultToCity !== null) {
            $this->query2 = $this->defaultToCity;
            $this->query2_search = $this->defaultToCity;
            $this->queryTo = $this->defaultToCity;
            $this->queryTo_search = $this->defaultToCity;
        }
    }
...

{
    ...
    {
        $place = app(GooglePlacesService::class)->reverseGeocode($latitude, $longitude);

        if (!$place) {
            $this->oneWayMsg = 'Current location could not be detected.';
            return;
        }
    ...
    {
        return app(GooglePlacesService::class)->placeDetails($placeId);
    }
